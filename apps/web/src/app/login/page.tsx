    ...
    <main className="relative overflow-hidden px-4 py-8 lg:px-8">
      <div className="mx-auto grid min-h-[calc(100vh-4rem)] max-w-[1420px] items-stretch gap-6 lg:grid-cols-[minmax(0,1.15fr)_470px]">
        <section
          ...
          className="relative overflow-hidden rounded-[32px] border border-white/40 bg-[linear-gradient(135deg,rgba(12,33,54,0.96),rgba(29,71,98,0.92))] p-7 text-white shadow-[0_30px_90px_rgba(12,33,54,0.18)] lg:p-10"
        >
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,rgba(198,125,47,0.18),transparent_25%),radial-gradient(circle_at_bottom_left,rgba(90,157,197,0.2),transparent_32%)]" />
          <div className="relative flex h-full flex-col justify-between gap-8">
            <div className="space-y-5">
              <div className="inline-flex items-center gap-3 rounded-full border border-white/10 bg-white/6 px-4 py-2">
                <span className="flex h-8 w-8 items-center justify-center rounded-full bg-white/12 text-xs font-black tracking-[0.2em]">
                  IX
                </span>
                <span className="text-xs font-bold uppercase tracking-[0.24em] text-slate-200/86">
                  Ilex Logistica
                </span>
              </div>

              <div className="space-y-4">
                <p className="text-[0.72rem] font-bold uppercase tracking-[0.28em] text-slate-300/75">
                  Torre de controle para exceções
                </p>
                <h1 className="max-w-3xl text-4xl font-black tracking-[-0.06em] text-white md:text-5xl">
            ...
            <div className="grid gap-4 lg:grid-cols-[1.2fr_0.9fr]">
              <div className="rounded-[28px] border border-white/10 bg-white/6 p-5 backdrop-blur">
                <p className="mb-4 text-[0.68rem] font-bold uppercase tracking-[0.22em] text-slate-300/74">
                  O que muda na rotina
                </p>
                <ul className="space-y-3">
                  {highlights.map((item) => (
                    <li key={item} className="flex items-start gap-3 text-sm leading-6 text-slate-100/88">
              ...
              <div className="rounded-[28px] border border-white/10 bg-slate-950/20 p-5 backdrop-blur">
                <p className="text-[0.68rem] font-bold uppercase tracking-[0.22em] text-slate-300/74">
                  Leituras essenciais
                </p>
                <div className="mt-4 grid gap-3">
                  <div className="rounded-2xl border border-white/8 bg-white/6 p-4">
                    <div className="text-[0.7rem] uppercase tracking-[0.2em] text-slate-300/70">Exceções críticas</div>
                    <div className="mt-2 text-3xl font-black tracking-[-0.05em]">24</div>
                    <div className="mt-1 text-sm text-slate-200/76">Foco imediato para tratativa</div>
                  </div>
                  <div className="rounded-2xl border border-white/8 bg-white/6 p-4">
                    <div className="text-[0.7rem] uppercase tracking-[0.2em] text-slate-300/70">SLA monitorado</div>
                    <div className="mt-2 text-3xl font-black tracking-[-0.05em]">92,4%</div>
                    <div className="mt-1 text-sm text-slate-200/76">Clareza para operação e liderança</div>
                  </div>
                </div>
              </div>
            </div>
