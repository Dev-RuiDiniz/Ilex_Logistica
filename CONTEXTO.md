...
- **2026-07-01:** Deploy na VPS `2.25.168.34` concluído — containers `ilex-db`, `ilex-api` e `ilex-web` operacionais; API respondendo `{"status":"ok"}` em `/health`; frontend acessível na porta 3000. Foram corrigidos durante o deploy: `alembic.ini` hardcoded em SQLite, `server_default` booleano da migration `sla_rules`, migration duplicada `alert_delivery_logs`, e imports/uso de `error-handler` removido no frontend. Tela de `/login` redesenhada com layout premium de duas colunas (preto/vermelho + formulário). Seed de demo aplicado: 4 usuários (`admin@ilex.com`, `logistica@ilex.com`, `gestor@ilex.com`, `auditoria@ilex.com`), todos com senha `123456`. Branch `feature/infra-vps-docker` contém todas as correções.
